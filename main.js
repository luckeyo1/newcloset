...
class ClosetItem extends HTMLElement {
    ...
    connectedCallback() {
        ...
        this.shadowRoot.innerHTML = `
            <style>
                :host { display: block; animation: slideUp 0.5s ease-out; }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const compressImage = (imgElement) => {
        return new Promise((resolve) => {
            ...
            const MAX_WIDTH = 500; // 용량을 더 줄이기 위해 500px로 조정
            ...
            resolve(canvas.toDataURL('image/jpeg', 0.6)); // 품질 0.6으로 최적화
        });
    };
    ...
    // --- Core Save Logic (WITH DETAILED ERROR FEEDBACK) ---
    form.addEventListener('submit', async (e) => {
        ...
        try {
            if (currentUser) {
                showToast('서버에 저장 시도 중...', 'info');
                // 상세 에러 캡처를 위한 저장 로직
                const userWardrobeRef = db.collection('wardrobes').doc(currentUser.uid).collection('items');
                await userWardrobeRef.add(itemData);
                showToast('로그인 계정에 저장되었습니다!', 'success');
            } else {
            ...
            // 구체적인 에러 메시지를 토스트로 출력
            showToast(`저장 실패 원인: ${err.code || err.message}`, 'error'); 
            if (err.code === 'permission-denied') {
                showToast('권한 거부: Firebase 콘솔에서 Firestore 규칙을 확인하세요.', 'error');
            }
        }
    });
    ...
    const syncTrialToCloud = async () => {
        ...
        if (items.length > 0 && currentUser) {
            for (const item of items) {
                delete item.id;
                await db.collection('wardrobes').doc(currentUser.uid).collection('items').add(item).catch(e => console.log('Sync err'));
            }
